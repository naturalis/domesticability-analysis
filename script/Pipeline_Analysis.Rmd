...
script only requires the 'dataset' variable and after it  is run, it will output
the 'predictors' variable.
...
# Construct model
GLM <- phyloglm(formula = finalFormula, data = dataset, phy = tree, method = "logistic_MPLE", btol = 36, log.alpha.bound = 4)
summary(GLM)
```
...
model1_Gaussian <- glm(Domestication ~ 1 + X5.1_AdultBodyMass_g + Horns_Antlers + AVGMovingSpeed, data = dataset, family = gaussian)
model1_Binomial <-  glm(Domestication ~ 1 + X5.1_AdultBodyMass_g + Horns_Antlers + AVGMovingSpeed, data = dataset, family = binomial)

# With DevelopmentStrategy
model2_Gaussian <- glm(Domestication ~ 1 + X5.1_AdultBodyMass_g + DevelopmentStrategy + Horns_Antlers + AVGMovingSpeed, data = dataset, family = gaussian)
model2_binomial <- glm(Domestication ~ 1 + X5.1_AdultBodyMass_g + DevelopmentStrategy + Horns_Antlers + AVGMovingSpeed, data = dataset, family = binomial)

check_model(model1_Gaussian)
